...
def get_score_1dimen(ground_truth_lst, lst, ground_truth_use_byte=True):
...
def get_score_2dimen(ground_truth_lst, lst, ground_truth_use_byte=True):
    if not ground_truth_use_byte:
        new_lst = [i * 2 for i in ground_truth_lst]
        ground_truth_lst = new_lst
...
if __name__ == "__main__":
    ...
    if not os.path.isdir(sys.argv[1]):
        print(f"input_file_dir {sys.argv[1]} is not a dir")
        sys.exit(0)
    ...
    ground_truth_file = os.path.join(
        sys.argv[4], os.path.split(in_dir_path)[1] + ".ground_truth.csv"
    )
    ground_truth = {}
    if not os.path.exists(ground_truth_file):
        ...
        header = next(reader)
        for row in reader:
            ground_truth[row[0]] = (ast.literal_eval(row[1]), row[2])
